...
```r
covid_data <- read.csv2("https://www.data.gouv.fr/api/1/datasets/r/fe3e7099-a975-4181-9fb5-2dd1b8f1b552")
head(covid_data)
```
...
#| eval: true
#| echo: false
head(read.csv("data/covid_data.csv"))
```
...
head(read.csv("data/hubeau_data.csv"))
```
...
#| eval: true
#| echo: false
head(read.csv("data/cities_table.csv"))
```
...
![](/pages/articles/images/ggplot2_ocr.png)

```{r ocr2}
#| echo: false
#| eval: false

# pour retrouver le code de ce graphe si besoin de l'améliorer
#library(ggplot2)
...
ggplot(
  plot,
  aes(x = nombre_pages, y = time_s, group = cluster, color = cluster)
) +
  geom_line() +
  geom_point() +
